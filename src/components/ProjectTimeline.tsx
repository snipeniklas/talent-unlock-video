...
const ProjectTimeline: React.FC<ProjectTimelineProps> = ({ onActivityClick, className = "" }) => {
  ...
  const activities: TimelineActivity[] = [
    {
      id: '1',
      title: String(get('home.hub.projectTimeline.activities.consultation.title')),
      duration: 0.5,
      startWeek: 0,
      status: 'completed',
      description: String(get('home.hub.projectTimeline.activities.consultation.description')),
      optional: true
    },
    {
      id: '2',
      title: String(get('home.hub.projectTimeline.activities.requirements.title')),
      duration: 0.2,
      startWeek: 0.5,
      status: 'active',
      description: String(get('home.hub.projectTimeline.activities.requirements.description'))
    },
    {
      id: '3a',
      title: String(get('home.hub.projectTimeline.activities.candidateProposal.title')),
      duration: 1,
      startWeek: 0.7,
      status: 'upcoming',
      description: String(get('home.hub.projectTimeline.activities.candidateProposal.description'))
    },
    {
      id: '3b',
      title: String(get('home.hub.projectTimeline.activities.selection.title')),
      duration: 0.8,
      startWeek: 1.7,
      status: 'upcoming',
      description: String(get('home.hub.projectTimeline.activities.selection.description'))
    },
    {
      id: '4',
      title: String(get('home.hub.projectTimeline.activities.implementation.title')),
      duration: 0.5,
      startWeek: 2.5,
      status: 'upcoming',
      description: String(get('home.hub.projectTimeline.activities.implementation.description'))
    }
  ];

  const totalWeeks = 3;
  const weekLabels = [
    String(get('home.hub.projectTimeline.weeks.optional')),
    String(get('home.hub.projectTimeline.weeks.week1')),
    String(get('home.hub.projectTimeline.weeks.week2')),
    String(get('home.hub.projectTimeline.weeks.week3'))
  ];
  ...
  const getDurationText = (duration: number): string => {
    const max = get('home.hub.projectTimeline.durationUnits.max');
    const dayText = get('home.hub.projectTimeline.durationUnits.day');
    const daysText = get('home.hub.projectTimeline.durationUnits.days');
    
    if (duration === 0.2) return `${max} 1 ${dayText}`;
    if (duration === 0.5) return `${max} 3 ${daysText}`;
    if (duration === 0.8) return `${max} 6 ${daysText}`;
    if (duration === 1) return `${max} 7 ${daysText}`;
    if (duration === 1.5) return `${max} 10 ${daysText}`;
    return `${max} ${Math.ceil(duration * 7)} ${daysText}`;
  };
      ...
      <CardHeader className="pb-4">
        <div className="flex items-center justify-between">
          <CardTitle className="text-xl md:text-2xl font-semibold text-brand-dark">
            {get('home.hub.projectTimeline.title')}
          </CardTitle>
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <Clock className="w-4 h-4" />
            <span>{get('home.hub.projectTimeline.duration')}</span>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-sm font-medium text-brand-dark">
                      ...
                      {activity.optional && (
                        <span className="text-xs text-muted-foreground ml-1">{get('home.hub.projectTimeline.optional')}</span>
                      )}
                    </span>
        ...
        <div className="flex justify-between items-center pt-4 border-t border-border">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-brand-dark">{get('home.hub.projectTimeline.milestones.kickoff')}</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 bg-green-500 rounded-full" />
            <span className="text-sm font-medium text-brand-dark">{get('home.hub.projectTimeline.milestones.firstDay')}</span>
          </div>
        </div>
        ...
        <div className="pt-4">
          ...
          >
            ...
            {get('home.hub.projectTimeline.cta')}
          </Button>
        </div>
      </CardContent>
    </Card>
