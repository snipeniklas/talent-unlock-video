...
const ProjectTimeline: React.FC<ProjectTimelineProps> = ({ onActivityClick, className = "" }) => {
  ...
  const activities: TimelineActivity[] = [
    {
      id: '1',
      title: t('components.projectTimeline.activities.consultation.title'),
      duration: 0.5,
      startWeek: 0,
      status: 'completed',
      description: t('components.projectTimeline.activities.consultation.description'),
      optional: true
    },
    {
      id: '2',
      title: t('components.projectTimeline.activities.requirements.title'),
      duration: 0.2,
      startWeek: 0.5,
      status: 'active',
      description: t('components.projectTimeline.activities.requirements.description')
    },
    {
      id: '3a',
      title: t('components.projectTimeline.activities.candidates.title'),
      duration: 1,
      startWeek: 0.7,
      status: 'upcoming',
      description: t('components.projectTimeline.activities.candidates.description')
    },
    {
      id: '3b',
      title: t('components.projectTimeline.activities.selection.title'),
      duration: 0.8,
      startWeek: 1.7,
      status: 'upcoming',
      description: t('components.projectTimeline.activities.selection.description')
    },
    {
      id: '4',
      title: t('components.projectTimeline.activities.implementation.title'),
      duration: 0.5,
      startWeek: 2.5,
      status: 'upcoming',
      description: t('components.projectTimeline.activities.implementation.description')
    }
  ];

  const totalWeeks = 3;
  const weekLabels = get<string[]>('components.projectTimeline.weekLabels', ['Opt. Woche 1', 'Woche 1', 'Woche 2', 'Woche 3']);
      ...
      <CardHeader className="pb-4">
        <div className="flex items-center justify-between">
          <CardTitle className="text-xl md:text-2xl font-semibold text-brand-dark">
            {t('components.projectTimeline.title')}
          </CardTitle>
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <Clock className="w-4 h-4" />
            <span>{t('components.projectTimeline.processWeeks').replace('{weeks}', '3')}</span>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
            ...
            <div key={activity.id} className="space-y-2">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-sm font-medium text-brand-dark">
                      ...
                      {activity.optional && (
                        <span className="text-xs text-muted-foreground ml-1">{t('components.projectTimeline.optional')}</span>
                      )}
                    </span>
                ...
                <div className="text-xs text-muted-foreground">
                  {activity.duration === 0.2 ? t('components.projectTimeline.duration.day') : 
                   t('components.projectTimeline.duration.days').replace('{days}', String(Math.ceil(activity.duration * 7)))}
                </div>
              </div>
        ...
        <div className="flex justify-between items-center pt-4 border-t border-border">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-brand-dark">{t('components.projectTimeline.milestones.kickoff')}</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 bg-green-500 rounded-full" />
            <span className="text-sm font-medium text-brand-dark">{t('components.projectTimeline.milestones.firstDay')}</span>
          </div>
        </div>
        ...
        <div className="pt-4">
          ...
          >
            ...
            {t('components.projectTimeline.cta')}
          </Button>
        </div>
      </CardContent>
    </Card>
